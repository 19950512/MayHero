    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-[radial-gradient(circle_at_top,#3a2b17_0%,#1b140d_30%,#100d08_70%,#0a0907_100%)] text-[#e8d7b2]">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-amber-100/40 text-sm mt-2">Faça login para continuar sua aventura</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-[#1a140f]/80 rounded-xl p-6 border border-amber-900/40 flex flex-col gap-4 backdrop-blur-sm">
          ...
          <p className="text-center text-amber-100/40 text-sm">
            Não tem conta?{' '}
            <Link href="/register" className="text-amber-300 hover:text-amber-200 font-semibold">Registre-se</Link>
          </p>
        </form>

        <div className="flex justify-center gap-6 mt-6 text-sm">
          <Link href="/rankings" className="text-amber-100/40 hover:text-amber-200 transition-colors">🏆 Rankings</Link>
          <Link href="/shop" className="text-amber-100/40 hover:text-amber-200 transition-colors">🛒 Shop</Link>
        </div>
      </div>
    </div>
  )
